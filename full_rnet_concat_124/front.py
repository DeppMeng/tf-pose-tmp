...
class HRFront():
    ...
    def __init__(self, num_channels, output_channels):
        self.scope = 'HR_FRONT'
    ...
    def forward(self, input):
        with tf.variable_scope(self.scope):
            ...
            _same_res = slim.conv2d(_out, num_outputs=self.output_channels[1], kernel_size=[3, 3],
                                    stride=1, activation_fn=tf.nn.relu, normalizer_fn=batch_norm)

            _half_res = slim.conv2d(_out, num_outputs=self.output_channels[2], kernel_size=[3, 3],
                                    stride=2, activation_fn=tf.nn.relu, normalizer_fn=batch_norm)

        return [_full_res, _same_res, _half_res]
