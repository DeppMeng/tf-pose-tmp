...
class HRModule():
    ...
    def forward(self, inputs):
        ...
        for i, _input in enumerate(inputs):
            _output_per_resolution = self.build_network_per_resolution(_input, channels=self.num_channels * pow(2, i), \
                                                                       scope=self.scope + '_B' + str(i), block_type=self.block_type)
    ...
    def build_network_per_resolution(self, input, channels, scope, block_type):
    ...
    def fuse_multi_resolution(self, features):
        ...
        dwfeatures = self.__get_dwsample_features(features)
        ...
        output_features = add_layers(origfeatures, dwfeatures, upfeatures, self.num_branches)
